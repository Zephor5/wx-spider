...
function init_clients() {
    try {
        ...
        for (var name in globalNames) {
            ...
            globalNames[name] = true;  // 过渡代码
            extraClients.push(new WxClient(wxClient.data_path, name, blogcache, _get_proxy()));
        }
    } catch (e) {
...
server.on('request', (function(req, res) {
    ...
    if (req_url.pathname.indexOf('/v1/wxspider/blogs') != -1) {
        ...
        if (!c.is_online()) {
            ...
            res.write('<!DOCTYPE html><html><head><title>' + c.clientName + '二维码</title></head><body><img style="width:276px;height:276px;" src="/v1/wxspider/img/' + c.clientName + '"></body></html>');
        } else {
            res.writeHead(200, {
                'Content-Type': 'text/plain'
            });
        ...
        res.end();
    } else if (req_url.pathname.indexOf('/v1/wxspider/img') != -1) {
        var c = wxClient,
            clientName = req_url.pathname.match(/\/v1\/wxspider\/img\/(.+)/);
        clientName = clientName ? clientName[1] : false;
        if (clientName === wxClient.clientName) {
            c = wxClient;
        } else {
