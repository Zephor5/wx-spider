...
var BlogCache = function(cache_path) {
  this._cache_path = cache_path?cache_path:'./' + '.cached_blogs.dat';
  this._cache={};
  try{
    fs.accessSync(this._cache_path, fs.R_OK | fs.W_OK);
    var data = fs.readFileSync(this._cache_path);
...
BlogCache.prototype.addBlog = function(publicID, blog) {
  ...
  fs.writeFile(this._cache_path, JSON.stringify(this._cache), function(err){
    if(err){
...
var blogcache = new BlogCache(args.blogCache);
var wxClient = new WxClient(args.qrcode,
                            args.clientName,
                            blogcache);
