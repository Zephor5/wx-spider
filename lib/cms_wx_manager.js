...
function init_clients() {
    try {
        ...
        var _d = fs.readFileSync(clients_data_path);
        globalNames = JSON.parse(_d);
        for(var name in globalNames){
            if(name === wxClient.clientName)
                continue;
            extraClients.push(new WxClient(wxClient.qrcode_path, name, blogcache));
        }
    } catch (e) {
...
function client_check() {
    if (!wxClient.status()) {
        serverLogger.warn('%s:重新登录中。。。', wxClient.clientName);
        wxClient.stop();
        wxClient.run();
    }
    for (var i = 0; i < extraClients.length; i++) {
        if (!extraClients[i].status()) {
        ...
        switch (action) {
            case 'add':
                if (clientName && !globalNames[clientName]) {
                    var c = new WxClient(wxClient.qrcode_path, clientName, blogcache);
                    extraClients.push(c);
