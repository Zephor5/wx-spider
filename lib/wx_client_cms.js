...
var WxClient = function(data_path, clientName, blogList) {
    ...
    var runtime_keys = ":deviceid:uuid:sid:uin:skey:pass_ticket:syncKey:syncStr:myUserName:cookies:members:groups:dn:";
    try {
...
WxClient.prototype.cleanup = function() {
    ...
    try {
        ...
        this._notice_log('write data file fail: %s', e.message);
    }
}
...
WxClient.prototype._notice_log = function(msg) {
    console.log('%s：%s', this.clientName, msg);
}
...
WxClient.prototype._wx_sync_check = function() {
    ...
    request.get({
        ...
        if (error) {
            ...
            if (!r) {
                clientLogger.error('Invaild body of synccheck:', body);
                this._wx_sync_check();
                return;
            }
            retcode = r[1];
