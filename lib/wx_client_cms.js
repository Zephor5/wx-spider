...
var WxClient = function(data_path, clientName, blogList, proxy) {
    ...
    if (fs.existsSync(this.runtime_data_file)) {
...
WxClient.prototype.clean_r = function() {
...
WxClient.prototype.is_running = function() {
    return +new Date - this.uptime < 35500;
}
...
WxClient.prototype._login_get_uuid = function() {
    ...
    var url = 'https://login.wx.qq.com/jslogin?appid=wx782c26e4c19acffb&redirect_uri=' + encodeURIComponent("https://wx.qq.com/cgi-bin/mmwebwx-bin/webwxnewloginpage") + "&fun=new&lang=zh_CN&_=" + this.dn++;
...
WxClient.prototype._login_check = function(tip) {
    ...
    var url = 'https://login.wx.qq.com/cgi-bin/mmwebwx-bin/login?' + querystring.unescape(login_check_query);
...
WxClient.prototype._wx_sync_check = function() {
    ...
    var url = 'https://webpush.wx.qq.com/cgi-bin/mmwebwx-bin/synccheck?' + querystring.stringify(query_dic);
    var headers = {
...
WxClient.prototype._wx_sync = function() {
    ...
    this.cr.post({
        ...
        if (msgBody['BaseResponse']['Ret'] != 0) {
            this._warn_log("获取消息错误：" + msgBody['BaseResponse']['Ret']);
            setTimeout(this._wx_sync_check.bind(this), 1000);
            return;
        }
        if (msgBody.SyncKey && msgBody.SyncKey.Count > 0) {
